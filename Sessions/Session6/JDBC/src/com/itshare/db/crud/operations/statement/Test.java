package com.itshare.db.crud.operations.statement;
...
public class Test {
	...
	void insert(int id , String desc){
		// quote + variable + quote 
		 String query ="insert into categories (id,description)  values ( "+id+",'"+desc+"');";
		...
		try {
			...
			// 3- execute statment / query
			int rowsAffcted =stat.executeUpdate(query);
	...
	void select(int x) {
		 String query ="select * from categories where id=" + x ;
	...
	void update(int id , String desc){
		// quote + variable + quote 
		 String query ="update categories set description ='"+desc+"' where id="+id+" ;";
